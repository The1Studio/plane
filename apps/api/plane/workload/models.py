...
class WorkloadCapacity(models.Model):
    """Per-member weekly capacity baseline, in hours.

    `project` is kept nullable for a future per-project override; v1 ALWAYS
    writes project=None (workspace-wide capacity — see plan D-B2). One row
    per (member, workspace, project).
    """
    ...
    # Denormalized scoping FK (mirrors WorkloadEstimate) — cheap filter, kept
    # in sync by the write path which always sets it from the slug.
    workspace = models.ForeignKey(
        "db.Workspace",
        on_delete=models.CASCADE,
        related_name="workload_capacities",
    ...
    weekly_hours = models.FloatField(validators=[MinValueValidator(0), MaxValueValidator(MAX_HOURS)])
    created_by = models.ForeignKey(
        settings.AUTH_USER_MODEL,
        null=True,
        blank=True,
    ...
    class Meta:
        ...
        constraints = [
            # Postgres treats NULL as DISTINCT in a plain UniqueConstraint
            # (Django 4.2 has no `nulls_distinct=False` — that's 5.0+), so a
            # single combined (member, workspace, project) constraint would
            # NOT stop duplicate workspace-wide (project=None) rows — which
            # is the ONLY row shape v1 ever writes. Two partial constraints
            # close that gap for both the null- and non-null-project cases.
    ...
    def __str__(self):
        return f"WorkloadCapacity(member={self.member_id}, weekly_hours={self.weekly_hours})"
