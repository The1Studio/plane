...
class WorkspaceCreateAPIEndpoint(BaseAPIView):
    ...
    def post(self, request):
        try:
            ...
            # Core's create() falls off the end here and returns None, which
            # blows up in DRF's finalize_response as a 500. Any other integrity
            # violation is still a write conflict — report it as one.
            return Response(
                {
                    "error": "The workspace could not be created",
                    "error_code": "WORKSPACE_CREATE_CONFLICT",
                },
                status=status.HTTP_409_CONFLICT,
            )
