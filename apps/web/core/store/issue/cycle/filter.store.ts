...
// the workspace store does for `search` (mirrors project-views/filter.store.ts).
...
export class CycleIssuesFilter extends IssueFilterHelperStore implements ICycleIssuesFilter {
  ...
  getAppliedFilters(cycleId: string) {
    ...
    // deliberately NEVER `updateFilters` (B2): a cycle's saved filters are SHARED, so a term
    // written back there would PATCH the cycle and change what every member sees. The term is
    ...
    // `"<EIssuesStoreType>:<entityId>"` (plan.md D3), so a cycle's term can never collide with a
    // module/project-view that happens to share its id. A blank/whitespace term returns
    ...
    const searchQuery = this.rootIssueStore.rootStore.viewsSearchStore.getSearchQuery(
      `${EIssuesStoreType.CYCLE}:${cycleId}`
    );
