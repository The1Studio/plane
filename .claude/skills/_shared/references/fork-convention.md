...
- **Backend** = NEW Django app under `apps/api/plane/<name>/` (owns its `migrations/`, `urls.py`,
  ...
  `project_ext`, `workspace_ext` — keep this list and the `forkApps` array below in sync. `forkApps` also selects which
...
```json
{
  ...
  "forkApps": ["ai_ext", "clickup_migrate", "workload", "github_ext", "project_ext", "workspace_ext"],
