...
6. Plane work-item update (see below)
...
Post the pass/fail counts to every work item bound to this session. A red suite does
NOT move the item backwards — a failing test is information, not a state regression.
If the project defines an extra `started`-group state named `In Review` / `Testing` /
`QA`, move there instead of commenting alone; never create such a state to satisfy
this. Degrades to a single warning when the `plane` MCP server is absent,
`T1K_PLANE_MODE` is `advisory`/`off`, or `--no-plane` was passed.
