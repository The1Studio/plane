...
function scanForSecrets(wiki) {
  // Patterns come from the kit-wide SSOT (hooks/telemetry-utils.cjs), which is a
  // strict superset of the five shapes this scanner used to carry inline. A hard
  // require is deliberate: if the SSOT cannot be resolved, a publish that would
  // otherwise run an unverified secret scan must fail loudly, not scan with a
  // silently narrower list.
  const { SECRET_TOKEN_PATTERNS } = require(
    path.join(__dirname, '..', '..', '..', 'hooks', 'telemetry-utils.cjs')
  );
  ...
  for (const { name, re } of SECRET_TOKEN_PATTERNS) {
    // SSOT patterns are shared module-level objects carrying /g, so `lastIndex`
    // persists between calls and `.exec()` on the shared object would skip every
    // other match. Clone without /g before probing.
    const probe = new RegExp(re.source, re.flags.replace('g', ''));
    const m = probe.exec(staged);
