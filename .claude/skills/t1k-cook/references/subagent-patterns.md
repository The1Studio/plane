...
## UI Implementation
```
Task(subagent_type="t1k-fullstack-developer", prompt="Implement [feature] UI per ./docs/design-guidelines.md", description="UI [feature]")
```
- For frontend work
...
`subagent_type` MUST be the resolved `reviewer` role (see § "Routing Resolution" above) — not the literal below. The brief MUST (a) declare the report output path — the reviewer's Deliverable-First Protocol (`agents/t1k-code-reviewer.md`) blocks on `AskUserQuestion` if none is given, a dead-end inside a Task subagent — and (b) inject acceptance criteria + touchpoints + public contracts so the reviewer can prove `HARD-GATE-NO-SIDE-EFFECTS` items 1/3/5 and the orchestrator can populate `review-decision.json`'s required `acceptanceCoverage` / `regressionProof` / `contractStatus` from a review that actually checked them — NOT from its own knowledge (that is the self-grading failure `artifact-gate-rules.md` exists to stop). These come from `context-snippets.json` (or the plan's requirements). A bare "review changes" brief is a violation.

```
Task(subagent_type="<resolved reviewer role>",
     prompt="Review the changes for [phase]. Write your report to [plan-dir]/reports/review-phase-[N].md.

Acceptance criteria — confirm the diff meets EACH:
[criteria from context-snippets.json / plan]

Touchpoints — walk each caller/dependent for regressions:
[touchpoints]

Public contracts that MUST stay stable — flag any change:
[signatures / exported types / API payloads / schemas / config keys]

Also check: security/OWASP, YAGNI/KISS/DRY, and no new lint/type/build errors.

Return: score (X/10); critical; warnings; suggestions; per-criterion acceptance coverage; touchpoint regression verdict; contract-stability status; clean build/lint/typecheck (pass|fail|not-run).",
...
- Acceptance / touchpoint / contract / clean-build map 1:1 to the 5 proofs in `hard-gates.md` § HARD-GATE-NO-SIDE-EFFECTS and the required fields of `review-decision.json` (`artifact-gate-rules.md`).
- `--parallel`: also inject the integration contract verbatim (`rules/contract-first-integration.md`) so the reviewer asserts both sides honor it.
...
- **Pass the PATH to the prior agent's report, not its pasted output.** Save the agent from re-running the same grep/read by naming `plans/reports/<prior-agent-report>.md` and letting it `Read` that — never paste the report body into the brief (it bloats the child's window before it does any work, and a pasted copy goes stale the moment the file changes). Per `rules/lean-brief-pointer-not-payload.md`.
