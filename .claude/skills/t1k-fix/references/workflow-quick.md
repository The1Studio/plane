...
Spawn the resolved `reviewer` role for quick review using the mandatory brief in `references/review-cycle.md` § "Reviewer Brief" (declares report path + injects root cause / blast radius / contracts). A bare "review the fix" prompt is a violation.
