...
| ---------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------ |
| `apps/web/core/components/issues/issue-detail/sidebar.tsx`                                           | "Estimated hours" property field (now reads/writes via the shared workload store); also renders the Progress bar (§`workload-progress.md`) alongside it | No upstream API to add an issue-property row; must render inside the core properties list  |
