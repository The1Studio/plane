...
| D14 | **One rule covers both invisibilities.** Driving rows off the member list gives a lane to a member with zero assigned items *and* to one whose items are all unestimated, with no second query and no special case — because from the reader's side the two are the same absence. |
...
| # | Decision |
| --- | --- |
| D1 | **A new `kind: "unscheduled"` block, one task per block.** Not a variant of `kind: "lane"`: a lane's box is `min(start)..max(target)` of its tasks, which for a same-day set is one column wide, and bars sized to `MIN_BAR_WIDTH` would overflow it. The unscheduled block spans the whole window like the header and footer do, and positions its single bar inside that box. |
...
| D6 | **Draggable and resizable**, gated by the same per-project `MEMBER`/`ADMIN` check the scheduled bars use. A drop writes both dates; a right-edge resize writes `target_date` and materialises `start_date` at the anchor. **Supersedes D8 of the scheduling plan**, which phase 6 rewrites rather than silently contradicts. |
...
| `apps/api/plane/workload/tests/test_member_rows.py` *(new)* | 1 |
