...
| D8  | **A task with no `start_date`** (bar drawn from `target_date` alone) gets both dates written on a move, and gains a `start_date` on a left-resize. A task with no `target_date` was originally never drawn and therefore never draggable; **that half is superseded** — it is now drawn as a dashed placeholder at `start_date ?? today` and is fully draggable and resizable. See `plans/260824-workload-unscheduled-in-today/`. The mechanism needed no change to `useTaskBarDrag`: the caller hands it a synthetic one-day task at the anchor, so this hook still only ever sees a dated task. |
