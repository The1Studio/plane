...
| D6  | **The server emits a `month_buckets` map (calendar month -> hours) alongside `buckets`**, and the badge's `used` reads it for month and quarter focus. Without this the badge is calendar-exact on the denominator and week-attributed on the numerator: at month zoom a week bucket is credited to the month its week STARTS in, so the `2026-08-31` bucket (Aug 31 - Sep 4) puts four September workdays into August's total and simultaneously removes them from September's. The client cannot repair this — at week granularity it only has week-summed hours. `spread_estimate` already walks day by day, so the second accumulation is exact and nearly free. |
...
| ----- | -------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------- |
| 1     | [`phase-1.md`](phase-1.md) | Backend: constants, model + migration, aggregation math, `month_buckets`, serializer/views/service, backend tests     | M (~6.5h) |
...
used     = week focus    -> sum of row.buckets whose day key is inside [focus.from, focus.to]
           month focus   -> row.month_buckets[focus month]
           quarter focus -> sum of row.month_buckets over the focus quarter's three months
```

Both sides are then measured over the SAME calendar range. `focusPeriodFor` already produces
exact calendar bounds — `monthRange` returns `2026-08-01 .. 2026-08-31` and `quarterRange`
returns `2026-07-01 .. 2026-09-30` — so "the month" means the 1st to the last day, never the
first week to the last week.

New response field (both API surfaces), sibling to `buckets` on every row:

```jsonc
"month_buckets": { "2026-08": 168.0, "2026-09": 42.5 }   // sparse, calendar months, always emitted
```

**Clipping caveat, inherited not introduced:** `month_buckets` counts only days inside the
requested `[date_from, date_to]` window, exactly as `buckets` does. A month the fetched window
covers only partly therefore reports partial hours against a full-month capacity. This is the
same limitation the current bucket-sum badge already has, and the store's range accumulation
(`merge.ts`) closes it as the user scrolls; it is recorded here so it is not mistaken for a
regression introduced by D6.
...
| Phase                                        | Effort    | Notes                                                         |
| -------------------------------------------- | --------- | ------------------------------------------------------------- |
| Phase 1: Backend daily hours + month_buckets | M (~6.5h) | Blocks Phase 2's end-to-end verification                      |
| Phase 2: Frontend rename + badge capacity    | M (~4.5h) | Depends on Phase 1                                            |
| Phase 3: Downstream propagation              | S (~1h)   | Depends on 1 + 2 green                                        |
| **Total**                                    | **~12h**  | Critical path: Phase 1 → Phase 2 → Phase 3 (fully sequential) |
...
- At month zoom, an assignee's badge reads BOTH sides over the calendar month: the denominator
  is that month's workday count x 8 (August 2026 → `168h`, not `200h`), and the numerator comes
  from `month_buckets` so it excludes the neighbouring month's days that share a week bucket.
